...
def test_generate_answer_rejects_unsupported_provider():
    ...
    with pytest.raises(GeneratorError, match="Unsupported LLM provider"):
        generate_answer("system", "user", settings)


def test_xai_provider_requires_api_key():
    settings = make_settings(llm_provider="xai", xai_api_key="")

    with pytest.raises(GeneratorError, match="XAI_API_KEY is required"):
        generate_answer("system", "user", settings)


def test_xai_response_parsing_uses_chat_completions(monkeypatch):
    calls = {}
